...
from .tracker.base_tracker_simple import BaseTracker
...
class TrackingAnything():
    ...
    def generator(self, images: list, template_mask:np.ndarray):
        ...
        for i in range(len(images)):
            ...
            # plt.show()
            if i ==0:           
                mask= self.xmem.track(images[i], template_mask)

                masks.append(mask)

                
            else:
                mask= self.xmem.track(images[i])
                # print(f'{i}th frame tracked')
                masks.append(mask)
            # plt.figure(figsize=(5, 5))
            # plt.imshow(mask, alpha=0.5)
            # plt.title(f'Tracked mask {i}')
            # plt.show()
        return masks
