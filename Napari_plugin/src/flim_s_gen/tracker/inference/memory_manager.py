...
class MemoryManager:
    ...
    def consolidation(self, candidate_key, candidate_shrinkage, candidate_selection, usage, candidate_value):
        ...
        # Make sure we don't ask for more prototypes than available
        k_proto = min(self.num_prototypes, usage.size(-1))
        _, max_usage_indices = torch.topk(usage, k=k_proto, dim=-1, sorted=True)
