...
class KMeansCluster(Container):
    def __init__(self, viewer: napari.viewer.Viewer):
        ...
        for d in ['G', 'S', 'Int1', 'Int2', 'Int3']:
            ...
            w = FloatSpinBox(min=0.0, max=10.0, step=0.1, value=1.0)
            row.append(w)
            self.append(row)
            self.weights[d] = w
...
def preprocess_rgb(img_stack, log_alpha, colormap, start_frame, end_frame, desc="Colorizing"):
    """
    Apply global (subrange-wide) log transform and colormap to frames [start_frame, end_frame].
    - 全局 min/max：先在子区间求全局极值，再对所有帧统一归一化
    - 归一化区间基于 log 之后的最大值（vmax_log）
    - tqdm 进度条
    """
